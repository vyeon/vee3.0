...
tcp_stream::tcp_stream(io::io_service& iosvc) :
    iosvc_ptr{ &iosvc },
    socket{ iosvc.kernel->to_boost() }
{
}

tcp_stream::tcp_stream(io::io_service& iosvc, tcp_socket&& __socket) :
    iosvc_ptr{ &iosvc },
...
size_t tcp_stream::write_some(const io::buffer& buffer, const size_t bytes_requested)
{
    ...
    while (bytes_transferred != bytes_requested)
    {
        ...
        {
            throw invalid_stream_exception{};
        }
        else if (boost::asio::error::connection_reset == error)
        {
            throw stream_reset_exception{};
        }
        else if (error)
        {
            throw unknown_io_exception{};
        }
    }
...
void tcp_stream::async_read_some(io::buffer buffer, size_t bytes_requested, io::async_io_callback callback) noexcept
{
    auto on_read = [this, buffer, callback](const ::boost::system::error_code& error, size_t bytes_transferred) -> void
    {
        ...
        {
            result.is_success = false;
            result.issue = io::io_issue::connection_reset;
        }
        else if (error)
        {
            result.is_success = false;
...
void tcp_stream::async_read_explicit(io::buffer buffer, size_t bytes_requested, io::async_io_callback callback) noexcept
{
    auto iteration = [this, buffer, bytes_requested, callback, total_transferred = static_cast<size_t>(0)](io::async_io_result& result) mutable -> void
    {
...
void tcp_stream::async_write_some(const io::buffer& buffer, size_t bytes_requested, io::async_io_callback callback) noexcept
{
...
void tcp_stream::async_read_some(io::buffer buffer, size_t bytes_requested, io::async_io_delegate::shared_ptr callback) noexcept
{
    auto binder = [callback = std::move(callback)](io::async_io_result& result) -> void
    {
        callback->do_call(result);
    };
...
void tcp_stream::async_read_explicit(io::buffer buffer, size_t bytes_requested, io::async_io_delegate::shared_ptr callback) noexcept
{
    auto binder = [callback = std::move(callback)](io::async_io_result& result) -> void
    {
        callback->do_call(result);
...
void tcp_stream::async_write_some(const io::buffer& buffer, size_t bytes_requested, io::async_io_delegate::shared_ptr callback) noexcept
{
    auto binder = [callback = std::move(callback)](io::async_io_result& result) -> void
    {
        callback->do_call(result);
...
io::io_service& tcp_stream::get_io_service() const noexcept
{
    return *iosvc_ptr;
}

tcp_server::tcp_server(io::io_service& iosvc, port_t port) :
    iosvc_ptr{ &iosvc },
    socket{ iosvc.kernel->to_boost() },
...
stream_socket::shared_ptr tcp_server::accept()
{
    ...
    stream_socket::shared_ptr session = std::make_shared<tcp_stream>(*iosvc_ptr, std::move(client));
    return session;
}
...
io::io_service& tcp_server::get_io_service() const noexcept
{
    return *iosvc_ptr;
}
...
{
    stream_socket::shared_ptr session = std::make_shared<tcp_stream>(iosvc);
    return session;
}

server_t::shared_ptr create_server(io::io_service& iosvc, port_t port) noexcept
{
    return std::make_shared<tcp_server>(iosvc, port);
}
